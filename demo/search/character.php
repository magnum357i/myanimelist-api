...
if ( \$mal->results !== FALSE ) {
	...
	foreach ( \$mal->results as \$result ) {
		...
		echo "<a href=\"" . \$mal->externalLink( 'character', \$result[ 'character_id' ] ) . "\" target=\"_blank\">{\$result[ 'character_name' ]}</a>";
		echo '</div>';

		if ( isset( \$result[ 'anime_titles_list' ] ) ) {

			echo '<div class="col">';
			echo 'A: ';
			echo '<ul class="commaList">';

			foreach( \$result[ 'anime_titles_list' ] as \$title ) {

				echo "<li><span class=\"badge badge-secondary\">{\$title}</span></li>";
			}
		...
		if ( isset( \$result[ 'manga_titles_list' ] ) ) {

			echo '<div class="col">';
			echo 'M: ';
			echo '<ul class="commaList">';

			foreach( \$result[ 'manga_titles_list' ] as \$title ) {
